...
def get_inventory():
    ...
    try:
        ...
        SELECT 
            ...
            COALESCE(wp.cena_sprzedazy_brutto, p.cena_sprzedazy_brutto, p.cena, 0) as price,
            COALESCE(wp.cena_sprzedazy_netto, p.cena_sprzedazy_netto, p.cena / (1 + COALESCE(p.stawka_vat, 23)/100.0), 0) as price_net,
            ...
            p.data_utworzenia as created_at,
            p.data_modyfikacji as updated_at,
            -- Informacje o cenach specjalnych
            CASE WHEN wp.id IS NOT NULL THEN 1 ELSE 0 END as has_special_price,
            wp.cena_sprzedazy_netto as special_price_netto,
            wp.cena_sprzedazy_brutto as special_price_brutto,
            COALESCE(p.cena_sprzedazy_brutto, p.cena, 0) as default_price_brutto
        FROM produkty p
        LEFT JOIN kategorie_produktow k ON p.category_id = k.id
        ...
        if location_id:
            ...
            # Dodaj warehouse_id jako pierwszy parametr dla JOIN z warehouse_product_prices
            params.insert(0, location_id)
            params.append(str(location_id))
            print(f"🔍 DEBUG dodaję filtr lokalizacji: {location_id}")
        else:
            # Jeśli nie ma location_id, użyj domyślnego warehouse_id = 5 dla cen specjalnych
            params.insert(0, 5)
        ...
        # Dodaj warehouse_id jako pierwszy parametr dla count query
        if location_id:
            count_params.append(location_id)
        else:
            count_params.append(5)  # Domyślny warehouse_id
