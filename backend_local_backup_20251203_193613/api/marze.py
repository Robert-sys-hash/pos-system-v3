        ...
        try:
            query = """
                SELECT 
                    ...
                    CASE 
                        WHEN p.cena_zakupu > 0 AND p.cena > 0 THEN 
                            ROUND(((p.cena - p.cena_zakupu) / p.cena) * 100, 2)
                        ELSE 0 
        ...
        try:
            ...
            cursor.execute("""
                SELECT 
                    ...
                    AVG(CASE 
                        WHEN cena_zakupu > 0 AND cena > 0 THEN 
                            ((cena - cena_zakupu) / cena) * 100
                        ELSE 0 
                    END) as avg_margin,
                    MIN(CASE 
                        WHEN cena_zakupu > 0 AND cena > 0 THEN 
            ...
            cursor.execute("""
                SELECT 
                    ...
                    ROUND(((cena - cena_zakupu) / cena) * 100, 2) as margin
                FROM produkty 
                WHERE cena_zakupu > 0 AND cena > 0 AND aktywny = 1
        ...
        try:
            ...
            cursor.execute("""
                SELECT 
                    ...
                    p.cena_zakupu,
                    p.cena as cena_sprzedazy,
                    ROUND(((p.cena - p.cena_zakupu) / p.cena) * 100, 2) as current_margin,
                ...
                AND p.cena > 0
                AND p.aktywny = 1
                AND ((p.cena - p.cena_zakupu) / p.cena) * 100 < ?
