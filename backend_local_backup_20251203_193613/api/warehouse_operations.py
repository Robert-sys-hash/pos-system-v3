...
from api.fifo_service import fifo_service, ProductBatch
...
def generate_external_receipt(invoice_id):
    ...
    try:
        ...
        for item in items:
            ...
            print(f"📊 Zaktualizowano stan w pos_magazyn dla produktu {item['produkt_id']}")
            
            # WAŻNE: Aktualizuj również inventory_locations (to używa frontend!)
            cursor.execute("""
                INSERT OR IGNORE INTO inventory_locations 
                (product_id, warehouse_id, ilosc_dostepna, ilosc_minimalna)
                VALUES (?, ?, 0, 0)
            """, (item['produkt_id'], warehouse_id))
            ...
            """, (item['ilosc'], item['produkt_id'], warehouse_id))
            
            print(f"📊 Zaktualizowano stan w inventory_locations dla produktu {item['produkt_id']} w magazynie {warehouse_id}")
            ...
            # 🆕 FIFO: Utwórz partię towarową dla przyjętej ilości
            try:
                batch = ProductBatch(
                    product_id=item['produkt_id'],
                    warehouse_id=warehouse_id,
                    received_date=datetime.now().isoformat(),
                    initial_quantity=item['ilosc'],
                    current_quantity=item['ilosc'],
                    purchase_price_net=item['cena_netto'],
                    purchase_price_gross=item['wartosc_brutto'] / item['ilosc'] if item['ilosc'] > 0 else 0,
                    purchase_invoice_id=invoice_id,
                    notes=f"Przyjęcie PZ z faktury {invoice['numer_faktury']}",
                    created_by='system'
                )
                
                batch_success, batch_message = fifo_service.create_batch(batch)
                if batch_success:
                    print(f"🎯 FIFO: Utworzono partię dla produktu {item['produkt_id']}: {batch_message}")
                else:
                    print(f"⚠️ FIFO: Błąd tworzenia partii dla produktu {item['produkt_id']}: {batch_message}")
                    
            except Exception as batch_error:
                print(f"⚠️ FIFO: Błąd systemu partii dla produktu {item['produkt_id']}: {str(batch_error)}")
                # Nie przerywamy procesu - FIFO jest opcjonalne
...
def get_active_inventory():
    ...
    try:
        ...
        SELECT 
            ...
            p.nazwa as product_name, p.kod_produktu as product_code, p.opis as description, p.jednostka as unit
        FROM inventory_sessions s
        LEFT JOIN inventory_items i ON s.id = i.session_id
        LEFT JOIN produkty p ON i.product_id = p.id
