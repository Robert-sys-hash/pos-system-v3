...
def create_app():
    ...
    try:
        ...
        error_msg = f"❌ Błąd podstawowych blueprintów: {e}"
        print(error_msg)
        app.blueprint_errors.append(error_msg)

    # 3. Blueprint autoryzacji - KRYTYCZNY!
    try:
        from api.auth import auth_bp
        app.register_blueprint(auth_bp, url_prefix='/api')
        print("✅ Auth blueprint OK")
    except Exception as e:
        error_msg = f"❌ Błąd auth blueprint: {e}"
        print(error_msg)
        app.blueprint_errors.append(error_msg)

    # 4. Dodatkowe blueprinty z obsługą błędów
    try:
    ...
    # Dodaj blueprint centralnego zarządzania marżą
    try:
        print("🔍 APP: Próbuję importować api.margins...")
        from api.margins import margins_bp
        print(f"🔍 APP: Zaimportowano margins_bp: {margins_bp}")
        print(f"🔍 APP: Blueprint name: {margins_bp.name}")
    ...
    # Dodaj blueprint systemu FIFO
    try:
        from api.fifo_api import fifo_bp
        app.register_blueprint(fifo_bp, url_prefix='/api')
        print("✅ FIFO system blueprint OK")
    except Exception as e:
        print(f"❌ Błąd FIFO blueprint: {e}")
