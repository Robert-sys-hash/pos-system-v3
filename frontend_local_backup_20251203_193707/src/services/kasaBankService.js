...
export const kasaBankService = {
  ...
  getSaldo: async (locationId = null) => {
    try {
      ...
      const url = locationId ? `saldo?location_id=${locationId}` : 'saldo';
  ...
  getDailySummary: async (date = null, locationId = null) => {
    try {
      ...
      let url = date ? `summary/daily?date=${date}` : 'summary/daily';
      if (locationId) {
  ...
  addOperation: async (operationData) => {
    try {
      const response = await api.post('operacje', operationData);
      return response.data;
    } catch (error) {
  ...
  getKPDocuments: async (limit = 50, offset = 0, dateFrom = null, dateTo = null, locationId = null) => {
    try {
      ...
      let url = `documents/kp?limit=${limit}&offset=${offset}`;
  ...
  getKWDocuments: async (limit = 50, offset = 0, dateFrom = null, dateTo = null, locationId = null) => {
    try {
      ...
      let url = `documents/kw?limit=${limit}&offset=${offset}`;
      if (dateFrom) url += `&date_from=${dateFrom}`;
      if (dateTo) url += `&date_to=${dateTo}`;
      if (locationId) url += `&location_id=${locationId}`;
  ...
  createOperacja: async (operacjaData) => {
    try {
      ...
      const response = await api.post('operacje', operacjaData);
  ...
  updateOperacja: async (operacjaId, operacjaData) => {
    try {
      ...
      const response = await api.put(`operacje/${operacjaId}`, operacjaData);
