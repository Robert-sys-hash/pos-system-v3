...
export const cenowkiService = {
  ...
  getProducts: async (limit = 50) => {
    try {
      const response = await api.get(`cenowki/products?limit=${limit}`);
  ...
  updateProductPrice: async (productId, newPrice) => {
    try {
      const response = await api.put(`cenowki/products/${productId}/price`, {
        price: newPrice
      });
      return response.data;
    } catch (error) {
  ...
  createOrUpdateCenowka: async (cenowkaData) => {
    try {
      const response = await api.post('cenowki', cenowkaData);
      return response.data;
    } catch (error) {
  ...
  getCenowkaByProduct: async (productId, locationId = null) => {
    try {
      const url = locationId 
        ? `cenowki/product/${productId}?location_id=${locationId}`
        : `cenowki/product/${productId}`;
