...
const SalesInvoicesPage = () => {
  ...
  const fetchInvoices = async () => {
    try {
      ...
      const response = await fetch(`https://panelv3.pl/api/sales-invoices${params}`);
  ...
  const fetchReceipts = async () => {
    try {
      let url = `https://panelv3.pl/api/receipts-for-invoicing?limit=${receiptsLimit}&offset=${receiptsOffset}`;
  ...
  const fetchReceiptDetails = async (receiptId) => {
    try {
      const response = await fetch(`https://panelv3.pl/api/receipt-details/${receiptId}`);
      const data = await response.json();
  ...
  const checkReceiptEligibility = async (receiptId) => {
    try {
      const response = await fetch(`https://panelv3.pl/api/receipt-invoice-eligibility/${receiptId}`);
      const data = await response.json();
  ...
  const fetchCustomers = async () => {
    try {
      const response = await fetch('https://panelv3.pl/api/customers');
      const data = await response.json();
  ...
  const previewInvoice = async (invoiceId) => {
    try {
      const response = await fetch(`https://panelv3.pl/api/sales-invoices/${invoiceId}`);
      const data = await response.json();
  ...
  // Pobierz PDF faktury z szablonem
  const downloadInvoicePDF = async (invoiceId, invoiceNumber, templateName = null) => {
    try {
      let blob;
      const finalTemplate = templateName || selectedTemplate;
      
      if (finalTemplate && templates.length > 0) {
        // Użyj szablonu
        blob = await invoiceTemplatesService.getInvoicePdfWithTemplate(invoiceId, finalTemplate);
      } else {
        // Użyj standardowego PDF
        blob = await invoiceTemplatesService.getInvoicePdf(invoiceId);
      }
      ...
      if (blob.type === 'application/pdf' || blob.size > 100) {
        ...
        a.href = url;
        const templateSuffix = finalTemplate ? `_${finalTemplate}` : '';
        a.download = `faktura_${invoiceNumber}${templateSuffix}.pdf`;
  ...
  const createInvoiceCorrection = async (invoiceId) => {
    ...
    try {
      const response = await fetch(`https://panelv3.pl/api/sales-invoices/${invoiceId}/correction`, {
        method: 'POST',
        headers: {
  ...
  const createInvoiceFromReceipt = async (receiptId, invoiceData) => {
    try {
      const response = await fetch(`https://panelv3.pl/api/sales-invoices/from-receipt/${receiptId}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([fetchInvoices(), fetchReceipts(), fetchCustomers(), loadTemplates()])
      .finally(() => setLoading(false));
  }, [receiptsLimit, receiptsOffset, filterDate, filterDateValue, currentLocationId]);

  // Ładuj szablony faktur
        ...
        <div className="col-12">
          ...
          {showTemplateSelector && (
            <div style={{
              ...
              <h5 style={{ marginBottom: '15px' }}>🎨 Opcje Szablonów PDF</h5>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px' }}>
                {templates.map((template) => (
                  <div key={template.id} style={{
                    padding: '10px',
                    border: selectedTemplate === template.id ? '2px solid #007bff' : '1px solid #ddd',
                    borderRadius: '4px',
                    backgroundColor: selectedTemplate === template.id ? '#e7f3ff' : 'white',
                    cursor: 'pointer'
                  }}
                  ...
                  >
                    ...
                    <div style={{ fontSize: '12px', color: '#666', marginBottom: '8px' }}>
                      {template.description || `Szablon ID: ${template.id}`}
                    </div>
                    <button
                      className="btn btn-sm btn-outline-primary"
                      onClick={(e) => {
                        e.stopPropagation();
                        window.open(invoiceTemplatesService.getTemplatePreviewUrl(template.id), '_blank');
                      }}
                      style={{ fontSize: '11px' }}
                    >
                      👁️ Podgląd
              ...
              <div style={{ marginTop: '10px', fontSize: '14px', color: '#666' }}>
                💡 Wybrany szablon będzie używany do wszystkich pobieranych PDF-ów faktur
              </div>
            </div>
          )}
                          ...
                          <tr key={invoice.id}>
                            ...
                            <td>
                              <div className="btn-group btn-group-sm" role="group">
                                ...
                                {/* Dropdown dla PDF z szablonami */}
                                <div className="btn-group" role="group">
                                  <button 
                                    className="btn btn-outline-success"
                                    onClick={() => downloadInvoicePDF(invoice.id, invoice.numer_faktury)}
                                    title={`Pobierz PDF (${templates.find(t => t.id === selectedTemplate)?.name || 'domyślny'})`}
                                  >
                                    📄
                                  </button>
                                  <button 
                                  ...
                                  <ul className="dropdown-menu">
                                    <li><h6 className="dropdown-header">🎨 Wybierz szablon:</h6></li>
                                    {templates.map((template) => (
                                      <li key={template.id}>
                                        <button 
                                          className={`dropdown-item ${template.id === selectedTemplate ? 'active' : ''}`}
                                          onClick={() => downloadInvoicePDF(invoice.id, invoice.numer_faktury, template.id)}
                                        >
                                          {template.name} {template.id === selectedTemplate && '✓'}
                                        </button>
                                      </li>
                                    ))}
                                    ...
                                    <li>
                                      <button 
                                        ...
                                        onClick={() => window.open(invoiceTemplatesService.getTemplatePreviewUrl(selectedTemplate), '_blank')}
                                      >
                                        👁️ Podgląd {templates.find(t => t.id === selectedTemplate)?.name || 'szablonu'}
                                      </button>
                                    </li>
                                  </ul>
                                </div>
...
const CreateInvoiceModal = ({ receipt, customers, eligibility, currentLocationId, onClose, onSubmit }) => {
  ...
  // Automatycznie wypełnij dane klienta z paragonu przy pierwszym załadowaniu
  useEffect(() => {
    if (receipt && receipt.klient_id) {
      // Wypełnij dane klienta z paragonu
      const customerName = receipt.nazwa_firmy || `${receipt.imie || ''} ${receipt.nazwisko || ''}`.trim();
      
      setSelectedCustomerId(receipt.klient_id);
      setCustomerSearchTerm(customerName);
      
      setFormData(prev => ({
        ...prev,
        nabywca_nazwa: customerName,
        nabywca_nip: receipt.klient_nip || '',
        nabywca_adres: receipt.ulica && receipt.numer_domu ? 
                      `${receipt.ulica} ${receipt.numer_domu}` : '',
        nabywca_miasto: receipt.miasto || '',
        nabywca_kod_pocztowy: receipt.kod_pocztowy || '',
        nabywca_telefon: receipt.telefon || '',
        nabywca_email: receipt.email || '',
        typ_faktury: receipt.nazwa_firmy ? 'firmowa' : 'imienna'
      }));
    }
  }, [receipt]);
