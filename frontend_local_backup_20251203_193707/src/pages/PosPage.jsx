  ...
  const loadAvailableDiscounts = async () => {
    try {
      ...
      const response = await fetch(
        `https://panelv3.pl/rabaty?user_id=${currentUser}`,
      );
  ...
  const applyDiscount = async (discountId, note = "") => {
    ...
    try {
      ...
      if (!transactionId) {
        ...
        for (const item of cart) {
          const addItemResponse = await fetch(
            `https://panelv3.pl/pos/cart/${transactionId}/items`,
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
      ...
      const discountResponse = await fetch(
        `https://panelv3.pl/pos/cart/${transactionId}/discount`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const removeDiscount = async (czasoweId) => {
    ...
    try {
      ...
      if (discountToRemove.uzycie_id && currentTransaction?.id) {
        const response = await fetch(
          `https://panelv3.pl/pos/cart/${currentTransaction.id}/discount/${discountToRemove.uzycie_id}`,
          {
            method: "DELETE",
          }
        );
