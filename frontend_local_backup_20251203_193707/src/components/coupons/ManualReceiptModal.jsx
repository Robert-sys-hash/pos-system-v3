...
function ManualReceiptModal({ coupon, isOpen, onClose, onSuccess }) {
  const [receiptNumber, setReceiptNumber] = useState(coupon?.numer_paragonu || '');
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    try {
      await couponService.updateReceiptNumber(coupon.id, receiptNumber);
      onSuccess && onSuccess();
      onClose();
    } catch (err) {
      setError('Błąd podczas zapisu numeru paragonu');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <span style={{ fontSize: 20, fontWeight: 600, letterSpacing: 0.2 }}>Edytuj numer paragonu</span>
          <button
            onClick={onClose}
            aria-label="Zamknij"
            style={{
