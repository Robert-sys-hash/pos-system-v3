...
const CorrectionModal = ({ isOpen, onClose, transaction, onCorrectionSubmit }) => {
  ...
  const loadTransactionDetails = async () => {
    ...
    try {
      const response = await fetch(`https://panelv3.pl/transactions/${transaction.id}/details`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`https://panelv3.pl/transactions/${transaction.id}/correction`, {
        method: 'POST',
        headers: {
