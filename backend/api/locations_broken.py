...
def get_locations():
    ...
    try:
        ...
        # Formatuj daty i pola dla kompatybilności z frontendem
        formatted_locations = []
        for location in locations:
            formatted_location = dict(location)
            
            # Normalizuj formaty dat do ISO
            if formatted_location.get('created_at'):
                created_at = formatted_location['created_at']
                if 'T' not in created_at:
                    # Konwertuj z 'YYYY-MM-DD HH:MM:SS' na 'YYYY-MM-DDTHH:MM:SS'
                    formatted_location['created_at'] = created_at.replace(' ', 'T')
            
            if formatted_location.get('updated_at'):
                updated_at = formatted_location['updated_at']
                if 'T' not in updated_at:
                    # Konwertuj z 'YYYY-MM-DD HH:MM:SS' na 'YYYY-MM-DDTHH:MM:SS'
                    formatted_location['updated_at'] = updated_at.replace(' ', 'T')
            
            # Normalizuj godziny_otwarcia
            godziny = formatted_location.get('godziny_otwarcia')
            if godziny == '{}' or godziny == '':
                formatted_location['godziny_otwarcia'] = None
            elif isinstance(godziny, str) and godziny.startswith('{'):
                try:
                    # Spróbuj sparsować jako JSON
                    import json
                    formatted_location['godziny_otwarcia'] = json.loads(godziny)
                except:
                    formatted_location['godziny_otwarcia'] = None
...
@locations_bp.route("/test")
def test_locations():
    return {"test": "locations blueprint works", "success": True}
