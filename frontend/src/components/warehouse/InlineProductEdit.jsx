    ...
    <div className="bg-white border rounded p-2 mb-2 shadow-sm" style={{ minWidth: '100%' }}>
      ...
      <form onSubmit={handleSubmit}>
        <div className="row g-1">
          ...
          {/* Marża */}
          {(parseFloat(formData.cena_sprzedazy_brutto) > 0 && parseFloat(formData.cena_zakupu_brutto) > 0) && (
            <div className="col-12">
              <div className="alert alert-info py-2 mb-2" style={{ fontSize: '0.8rem' }}>
                <i className="fas fa-calculator me-2"></i>
                <strong>Marża:</strong> {(((parseFloat(formData.cena_sprzedazy_brutto) - parseFloat(formData.cena_zakupu_brutto)) / parseFloat(formData.cena_zakupu_brutto)) * 100).toFixed(1)}%
                <span className="ms-3">
                  <strong>Zysk:</strong> {(parseFloat(formData.cena_sprzedazy_brutto) - parseFloat(formData.cena_zakupu_brutto)).toFixed(2)} zł
                </span>
                <span className="ms-3">
                  <strong>VAT:</strong> {(parseFloat(formData.cena_sprzedazy_brutto) - parseFloat(formData.cena_sprzedazy_netto)).toFixed(2)} zł
                </span>
              </div>
            </div>
          )}
