...
export const warehousePricingService = {
  ...
  getWarehousePrices: async (warehouseId) => {
    try {
      const response = await api.get(`/api/warehouses/${warehouseId}/prices`);
  ...
  setWarehousePrice: async (warehouseId, productId, priceData) => {
    try {
      const response = await api.put(`/api/warehouses/${warehouseId}/products/${productId}/price`, priceData);
      return response.data;
    } catch (error) {
  ...
  getCurrentPrice: async (warehouseId, productId) => {
    try {
      const response = await api.get(`/api/warehouses/${warehouseId}/products/${productId}/price`);
      return response.data;
    } catch (error) {
  ...
  removeWarehousePrice: async (warehouseId, productId, dataOd) => {
    try {
      const response = await api.delete(`/api/warehouses/${warehouseId}/products/${productId}/price`, {
        data: { data_od: dataOd }
      });
      return response.data;
    } catch (error) {
  ...
  getPriceHistory: async (warehouseId, productId) => {
    try {
      const response = await api.get(`/api/warehouses/${warehouseId}/products/${productId}/price-history`);
      return response.data;
    } catch (error) {
  ...
  copyPricesBetweenWarehouses: async (sourceWarehouseId, targetWarehouseId) => {
    try {
      const response = await api.post(`/api/warehouses/${sourceWarehouseId}/copy-prices-to/${targetWarehouseId}`);
      return response.data;
    } catch (error) {
