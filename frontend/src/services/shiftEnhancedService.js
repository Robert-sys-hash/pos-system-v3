...
export const shiftEnhancedService = {
  ...
  async openShiftEnhanced(shiftData) {
    try {
      const response = await api.post('/api/shifts/open-enhanced', shiftData);
  ...
  async closeShiftEnhanced(closeData) {
    try {
      const response = await api.post('/api/shifts/close-enhanced', closeData);
      return response.data;
    } catch (error) {
  ...
  async getDailyClosureReports(filters = {}) {
    try {
      ...
      const response = await api.get(`/api/admin/daily-closure-reports?${params}`);
      return response.data;
    } catch (error) {
  ...
  async getDailyClosureReportDetails(reportId) {
    try {
      const response = await api.get(`/api/admin/daily-closure-reports/${reportId}`);
      return response.data;
    } catch (error) {
