...
class CustomTemplateManager:
            ...
            if field_type == 'text':
                ...
                if table_type == 'vat_summary':
                    ...
                    if invoice_data.get('vat_summary'):
                        ...
                        # Twórz tabelę VAT z szerszymi kolumnami
                        vat_table = Table(vat_data, colWidths=[80, 120, 120, 120])
                    ...
                    if positions:
                        ...
                        # Twórz tabelę z szerszymi kolumnami dla cen
                        table = Table(table_data, colWidths=[25, 180, 40, 100, 100])
                        table.setStyle(TableStyle([
                            ('BACKGROUND', (0, 0), (-1, 0), colors.grey),
                            ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
                            ('ALIGN', (0, 0), (-1, -1), 'CENTER'),
                            ('ALIGN', (0, 1), (1, -1), 'LEFT'),  # Nazwy produktów do lewej
                            ('VALIGN', (0, 0), (-1, -1), 'TOP'),  # Wyrównanie do góry
                            ('FONTNAME', (0, 0), (-1, 0), 'Helvetica-Bold'),
                            ('FONTSIZE', (0, 0), (-1, 0), 12),
                            ('FONTSIZE', (0, 1), (-1, -1), 10),  # Mniejsza czcionka w danych
                            ('BOTTOMPADDING', (0, 0), (-1, 0), 12),
    ...
    def _create_custom_items_table(self, positions, config, styles):
        ...
        # Szerokości kolumn - szersze dla cen
        col_widths = []
        if show_row_numbers:
            col_widths.append(1*mm*10)  # Lp.
        col_widths.extend([7*mm*10, 2*mm*10, 3*mm*10, 3*mm*10])  # Nazwa, Ilość, Cena, Wartość
